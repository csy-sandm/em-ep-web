...
// 引入API
import {
  alarmCaseInfoQueryListByPage,
  alarmCaseInfoQueryList,
  alarmCaseInfoInsertList,
  alarmCaseInfoInsert,
  alarmCaseInfoUpdateList,
  alarmCaseInfoUpdate,
  alarmCaseInfoDelete
} from '@/api/em-ep/alarmCaseInfoApi.js'
...
export default {
  data () {
    return {
      ...
      // 必填字段 前面加'*'
      rules: {
        caseId: [{ required: true, message: '请输入', trigger: 'blur' }]
      }
  ...
  methods: {
    ...
    async getDataList () {
      ...
      alarmCaseInfoQueryListByPage(this.queryParam).then((response) => {
    ...
    async insertData (insertParam) {
      alarmCaseInfoInsert(insertParam).then((response) => {
        const resultCode = response.resultCode
        if (resultCode === '2000') {
          // 这里根据插入结果，页面提示
          alert(response.resultMsg)
    ...
    async updateData (editParam) {
      alarmCaseInfoUpdate(editParam).then((response) => {
        const resultCode = response.resultCode
        if (resultCode === '2000') {
          // 这里根据插入结果，页面提示
          alert(response.resultMsg)
    ...
    async handleDel () {
      const params = {
        caseId: this.delParam.caseId
      }
      alarmCaseInfoDelete(params).then((response) => {
        const resultCode = response.resultCode
        if (resultCode === '2000') {
          // 这里根据插入结果，页面提示
          alert(response.resultMsg)
    ...
    async exportExecl () {
      ...
      const params = {}
      alarmCaseInfoQueryList(params).then((response) => {
        // 数据
        this.tableData = response.resultEntity
        console.log(this.tableData)
        outExportExcel(this.tableHeader, this.tableKey, this.tableData, '告警方案')
      })
    },
