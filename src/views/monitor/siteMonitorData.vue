	...
	<div>
		...
		<el-dialog title="新增信息"
			...
			<el-form ref="form" label-width="200px" :model="insertParam"  :rules="rules"  >
				...
				<el-form-item label="站点编码" style="width: 500px" prop="siteId" >
...
} from '@/api/em-ep/siteMonitorDataApi.js'
...
export default {
  ...
  methods: {
    ...
    async getDataList () {
      ...
      siteMonitorDataQueryListByPage(this.queryParam).then((response) => {
    ...
    async insertData (insertParam) {
      siteMonitorDataInsert(insertParam).then((response) => {
        const resultCode = response.resultCode
        if (resultCode === '2000') {
          // 这里根据插入结果，页面提示
          alert(response.resultMsg)
    ...
    async exportExecl () {
      ...
      const params = {}
      siteMonitorDataQueryList(params).then((response) => {
        // 数据
